...
const Register:React.FC<Props> = ({}) => {
 ...
 const handleForm:React.FormEventHandler<HTMLFormElement> = async (e:any)=> {
  ...
  setGif(true)
  e.target.reset()
  const response = await register(form[0])
  if (response.data?.register.errors) {
    setError(true)
    if (response.data.register.errors[0].field === "username"){
      setUsernameError(response.data.register.errors[0].message)
    }
    else if(response.data.register.errors[0].field === "password"){
      setPasswordError(response.data.register.errors[0].message)
    } 
 ...
 return(
  <div className="max-w-5xl flex flex-col justify-center items-center space-y-2">
    ...
    <h1 className="text-center font-extrabold text-3xl">Register your account</h1> 
    ...
    <form onSubmit={handleForm} className="w-full">
      ...
      <button disabled={!form} type="submit" className="flex flex-row w-full h-10 bg-orange-500 mt-2 rounded-[8px]">
        <span className="p-2">
        ...
        {gif? <Loading />
        :<h1 className="p-2 mx-auto text-center text-white text-md font-bold">Click to register</h1>
      }
      </button>
      </form>
  </div>
